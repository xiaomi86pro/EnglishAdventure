...
class Question8 {
  ...
  async generateQuestion(enemyType) {
      try {
          ...
          try {
              compatibleCategories = JSON.parse(verb.compatible_objects);
          } catch (e) {
              console.error('[Question8] Invalid JSON in compatible_objects:', e);
              return;
          }

          if (!compatibleCategories || compatibleCategories.length === 0) {
              console.error('[Question8] No compatible categories for verb:', verb.base_form);
              return;
          }
